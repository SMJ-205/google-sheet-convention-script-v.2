...
function checkTriggers() {
  ...
  const crashErr =
    PropertiesService.getScriptProperties().getProperty("ONCHANGE_ERROR");
  if (crashErr) {
    msg += "\n\n⚠ LATEST CRASH:\n" + crashErr;
  }

  const debugLog =
    PropertiesService.getScriptProperties().getProperty("ONCHANGE_DEBUG");
  if (debugLog) {
    msg += "\n\nℹ LATEST RUN:\n" + debugLog;
  }
...
function _handleEdit_(e, useAlerts) {
  ...
  if (sheetName === "Schema") {
    try {
      ...
      if (tableCell)
        CacheService.getScriptCache().remove("schema_" + tableCell);
    } catch (_) {}
    return;
  }
...
function validateInputs() {
  ...
  const cache = CacheService.getScriptCache();
  const schema =
    JSON.parse(cache.get("schema_" + sheetName) || "null") ||
    fetchAndCacheSchema(sheetName, cache);

  if (!schema) {
    SpreadsheetApp.getUi().alert(
      "No schema found. Run 'Generate / Update Schema' first.",
    );
    return false;
  }
  ...
  headers.forEach((h, i) => {
    ...
    if (safe)
      colArrays[safe] = data.map((r) => String(r[i]).toLowerCase().trim());
  });
  ...
  data.forEach((rowData, r) => {
    ...
    const hasData = rowData.some(
      (v, i) => i !== uIdx && v !== "" && v !== null && v !== undefined,
    );
    if (!hasData) return;
    ...
    headers.forEach((colName, c) => {
      ...
      if (!isEmpty) {
        ...
        // 3. Unique
        if (rule.is_unique) {
          const count = (colArrays[safe] || []).filter(
            (v) => v === String(val).toLowerCase().trim(),
          ).length;
  ...
  failed.forEach((f) => {
    f.cells.forEach((fc) => {
      sheet
        .getRange(f.row, fc.colIdx + 1)
        .setBackground(CONFIG.soft_rejection_color);
    });
  });
  ...
  if (failed.length) {
    ...
    failed.slice(0, 8).forEach((f) => {
      const cellRefs = f.cells
        .map(
          (fc) => toColLetter(fc.colIdx + 1) + f.row + " (" + fc.reason + ")",
        )
        .join(", ");
      alertMsg += "  Row " + f.row + ": " + cellRefs + "\n";
    });
    if (failed.length > 8)
      alertMsg += "  ...and " + (failed.length - 8) + " more.";
  }
  ...
  if (failed.length) {
    ...
    failed.forEach((f) => {
      f.cells.forEach((fc) => {
        ...
        errorLines.push(
          "Row " + f.row + " &rarr; <b>" + colName + "</b>: " + reasonLabel,
        );
      });
    });
    _showErrorDialog_(
      "Validation Errors (" + failed.length + " row(s) failed)",
      errorLines,
    );
  }
...
function _showErrorDialog_(title, errorLines) {
  const rows = errorLines
    .map(
      (l) =>
        '<li style="padding:6px 0;border-bottom:1px solid #f0f0f0;line-height:1.5">' +
        l +
        "</li>",
    )
    .join("");
  ...
  const html =
    ...
    '  <h3 style="margin:0 0 12px;color:#c62828;font-size:15px;">' +
    title +
    "</h3>" +
...
function _friendlyReason_(reason) {
  if (reason === "mandatory")
    return '<span style="color:#c62828">⚠ Field is mandatory — cannot be empty</span>';
  if (reason === "not unique")
    return '<span style="color:#e65100">⚠ Value is not unique — duplicate found</span>';
  if (reason.startsWith("type:")) {
    const t = reason.replace("type:", "");
    return (
      '<span style="color:#6a1b9a">⚠ Invalid type — expected ' + t + "</span>"
    );
  }
  return reason;
}
